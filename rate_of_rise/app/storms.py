...
SHARE_SUBDIR = "rate_of_rise"

# The add-on's previous slug, under which this subdirectory lived before the Rate of Rise
# rename. An install that predates the rename has its storm log here; migrated the same
# way the /data -> /share move already is, below.
LEGACY_SHARE_SUBDIR = "creek_modeling"
...
def _resolve_db_path(data_dir: Path, share_dir: Path | None) -> Path:
    ...
    if not target.exists():
        # Prefer a log under the add-on's previous slug (post-rename) over the older
        # /data fallback (pre-/share) — either way, move rather than copy.
        renamed = share_dir / LEGACY_SHARE_SUBDIR / "events.sqlite"
        source = renamed if renamed.exists() else (legacy if legacy.exists() else None)
        if source is not None:
            try:
                shutil.move(str(source), str(target))
                log.info("Migrated storm event log %s -> %s", source, target)
            except OSError as exc:
                log.warning("Could not migrate %s (%s) — staying put", source, exc)
                return source
    return target
