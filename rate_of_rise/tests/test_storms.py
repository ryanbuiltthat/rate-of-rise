...
Run: python rate_of_rise/tests/test_storms.py
"""
import shutil
import sqlite3
import sys
...
def test_a_log_under_the_pre_rename_share_subdir_is_migrated_too():
    import gc

    old_data = Path(tempfile.mkdtemp())
    share = Path(tempfile.mkdtemp())
    StormLog(old_data, None).observe(row(T0, rain=0.5))   # a log that predates the rename
    gc.collect()   # Windows: drop the sqlite3 connection's handle before the OS-level move
    old_dir = share / st.LEGACY_SHARE_SUBDIR
    old_dir.mkdir(parents=True)
    shutil.move(str(old_data / "events.sqlite"), str(old_dir / "events.sqlite"))

    s = StormLog(Path(tempfile.mkdtemp()), share)
    moved = share / st.SHARE_SUBDIR / "events.sqlite"
    assert s.path == moved and moved.exists()
    assert not (old_dir / "events.sqlite").exists()
    assert s.count(completed_only=False) == 1
