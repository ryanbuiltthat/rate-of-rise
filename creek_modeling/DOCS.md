...
**Creek Modeling** device with no package or `configuration.yaml` edit, and they
...
- **Entity IDs:** the discovered entities belong to an *Creek Modeling* device, so
  Home Assistant prefixes the device name:
  `sensor.creek_modeling_creek_flood_probability`,
  `sensor.creek_modeling_creek_qpf_24h`, and so on. Entities defined in
