...
All notable changes to the **Creek Modeling** add-on are documented here.
...
- **USGS downstream gauges — ingestion slice 2c**: new `app/sources/usgs.py` polls NWIS
  instantaneous values for `<usgs downstream>` (receiving river, downstream city) and
  `<usgs adjacent>` (regional Creek), reporting gage height, discharge, and 3 h rise for each. Free,
  no key. the adjacent creek drains the same upstream upland as our upstream
