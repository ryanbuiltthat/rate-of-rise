...
def test_dashboard_creek_entities_are_published_by_the_gateway():
    ...
    `creek_modeling_` device prefix, and test_dashboard_entities.py owns those.
    `sensor.creek_modeling_*` is likewise the add-on's, not the gateway's.
    """
    ...
    referenced = {e for e in re.findall(r"\bsensor\.creek_[a-z0-9_]+\b", dash)
                  if not e.startswith("sensor.creek_modeling_")}
